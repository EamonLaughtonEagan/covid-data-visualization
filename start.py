...
app.layout = html.Div([
    ...
    dcc.RangeSlider(
        ...
        marks={
            ...
            len(df) / 2: str(df['REPORT_DATE.MONTH'][(len(df) - 1) / 2]) + ' ' + str(
                df['REPORT_DATE.DAY'][(len(df) - 1) / 2]),
...
def update_node_graph(slider_range):
    ...
    fig.update_layout(
        ...
        legend=dict(
            x=1, y=1, title_text='Locations', bordercolor='black', borderwidth=2, itemclick='toggleothers',
        ),
...
def update_dates(slider_range):
    low, high = slider_range
    container = df['REPORT_DATE.MONTH'][low] + ' ' + str(df['REPORT_DATE.DAY'][low]) + ' - ' + df['REPORT_DATE.MONTH'][
        high] + ' ' + str(df['REPORT_DATE.DAY'][high])
    return container
